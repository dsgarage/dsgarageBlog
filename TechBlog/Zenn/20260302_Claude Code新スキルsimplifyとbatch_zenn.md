---
title: "Claude Code新スキル /simplify と /batch — Unity開発を加速する2つの自動化"
emoji: "🔧"
type: "tech"
topics: ["Unity", "ClaudeCode", "AI", "開発効率化", "ゲーム開発"]
published: false
---
...
- **`/simplify`** は「PRに出す前の30分の手作業」を自動化する
- **`/batch`** は「丸1日かかるマイグレーション」を並列で数十分に短縮する
- どちらも「コードの機能は変えずに、品質と開発速度を上げる」という設計思想
- `/batch` は内部で `/simplify` を自動実行するため、生成PRは品質チェック済み
- Unity開発のように大量のスクリプトファイルを扱うプロジェクトで特に効果的
